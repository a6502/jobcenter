...
-- /usr/lib/postgresql/9.6/bin/pg_dump --clean --if-exists jobcenter
...
DROP INDEX IF EXISTS jobcenter.workers_stopped_idx;
DROP INDEX IF EXISTS jobcenter.jobs_timeout_idx;
DROP INDEX IF EXISTS jobcenter.jobs_state_idx;
DROP INDEX IF EXISTS jobcenter.job_task_log_jobid_idx;
DROP INDEX IF EXISTS jobcenter.job_parentjob_id_index;
...
ALTER TABLE IF EXISTS ONLY jobcenter.tasks DROP CONSTRAINT IF EXISTS tasks_pkey;
...
ALTER TABLE IF EXISTS ONLY jobcenter.json_schemas DROP CONSTRAINT IF EXISTS json_schemas_pkey;
...
ALTER TABLE IF EXISTS jobcenter.next_tasks DROP CONSTRAINT IF EXISTS check_same_workflow;
...
DROP TABLE IF EXISTS jobcenter.json_schemas;
...
DROP TABLE IF EXISTS jobcenter.doodat;
...
DROP FUNCTION IF EXISTS jobcenter.do_workflowoutargsmap(a_jobtask jobtask);
...
DROP FUNCTION IF EXISTS jobcenter.do_validate_json_schema_type(type text, data jsonb);
DROP FUNCTION IF EXISTS jobcenter.do_validate_json_schema(schema jsonb, data jsonb, root_schema jsonb);
...
DROP FUNCTION IF EXISTS jobcenter.do_task_error(a_jobtask jobtask, a_outargs jsonb);
...
DROP FUNCTION IF EXISTS jobcenter.do_sleep_task(a_jobtask jobtask);
...
DROP FUNCTION IF EXISTS jobcenter.do_populate_env(a_jobtask jobtask, a_env jsonb);
...
DROP FUNCTION IF EXISTS jobcenter.do_outargscheck(a_action_id integer, a_outargs jsonb);
...
DROP FUNCTION IF EXISTS jobcenter.do_inargsmap(a_action_id integer, a_jobtask jobtask, a_args jsonb, a_env jsonb, a_vars jsonb);
DROP FUNCTION IF EXISTS jobcenter.do_inargscheck(a_action_id integer, a_args jsonb);
...
DROP FUNCTION IF EXISTS jobcenter.blabla(a_jobtask jobtask, a_foo jsonb, a_bar text[]);
